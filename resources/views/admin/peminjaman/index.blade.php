...
@section('content')
    <div class="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        ...
        {{-- Summary Cards Grid --}}
        <div class="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-4 animate-fade-up animate-duration-[700ms] animate-delay-75 animate-ease-out">
            @foreach ($ringkasan as $status => $total)
                @php
                    $statusLower = strtolower($status);
                    $dotColor = match (true) {
                        str_contains($statusLower, 'setuju') || str_contains($statusLower, 'disetujui') => 'bg-emerald-400 shadow-emerald-500/50',
                        str_contains($statusLower, 'tolak') || str_contains($statusLower, 'ditolak') => 'bg-rose-400 shadow-rose-500/50',
                        str_contains($statusLower, 'selesai') => 'bg-sky-400 shadow-sky-500/50',
                        default => 'bg-amber-400 shadow-amber-500/50',
                    };
                    $badgeStyle = match (true) {
                        str_contains($statusLower, 'setuju') || str_contains($statusLower, 'disetujui') => 'text-emerald-400 border-emerald-500/20 bg-emerald-500/10',
                        str_contains($statusLower, 'tolak') || str_contains($statusLower, 'ditolak') => 'text-rose-400 border-rose-500/20 bg-rose-500/10',
                ...
                @endphp
                <div class="rounded-2xl border border-gray-800 bg-gray-900 p-5 shadow-lg transition-all hover:border-gray-700">
        ...
        {{-- Filter Card --}}
        <div class="mb-8 rounded-2xl border border-gray-800 bg-gray-900 p-6 shadow-xl animate-fade-up animate-duration-[800ms] animate-delay-100 animate-ease-out">
            <div class="mb-5 flex items-center gap-2 border-b border-gray-800/80 pb-3 text-xs font-semibold uppercase tracking-wider text-amber-400">
                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
            ...
            <form method="GET" action="{{ route('admin.peminjaman.index') }}" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-6 items-end">

                {{-- Status --}}
                <div class="lg:col-span-1">
                    <label for="status" class="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-400">Status</label>
                    <div class="relative">
                        <select
                            id="status"
                            name="status"
                        ...
                        >
                            ...
                            @foreach (\App\Enums\StatusPeminjaman::cases() as $status)
                                <option value="{{ $status->value }}" class="bg-gray-900 text-white" @selected(($filters['status'] ?? '') === $status->value)>
                                    {{ $status->value }}
                                </option>
                            @endforeach
                        </select>
                ...
                {{-- Ruangan --}}
                <div class="lg:col-span-1">
                    <label for="id_ruangan" class="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-400">Ruangan</label>
                    <div class="relative">
                        <select
                            id="id_ruangan"
                            name="id_ruangan"
                        ...
                        >
                            ...
                            @foreach ($ruangan as $item)
                                <option value="{{ $item->id_ruangan }}" class="bg-gray-900 text-white" @selected((string) ($filters['id_ruangan'] ?? '') === (string) $item->id_ruangan)>
                                    {{ $item->nama_ruangan }}
                                </option>
                            @endforeach
                        </select>
                ...
                {{-- Peminjam --}}
                <div class="lg:col-span-1">
                    <label for="id_user" class="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-400">Peminjam</label>
                    <div class="relative">
                        <select
                            id="id_user"
                            name="id_user"
                        ...
                        >
                            ...
                            @foreach ($users as $item)
                                <option value="{{ $item->id_user }}" class="bg-gray-900 text-white" @selected((string) ($filters['id_user'] ?? '') === (string) $item->id_user)>
                ...
                {{-- Tanggal Mulai --}}
                <div class="lg:col-span-1">
                    <label for="tanggal_mulai" class="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-400">Tgl Mulai</label>
                    <input
                        id="tanggal_mulai"
                        name="tanggal_mulai"
                        type="date"
                ...
                {{-- Tanggal Selesai --}}
                <div class="lg:col-span-1">
                    <label for="tanggal_selesai" class="mb-2 block text-xs font-semibold uppercase tracking-wider text-gray-400">Tgl Selesai</label>
                    <input
                        id="tanggal_selesai"
        ...
        <div class="overflow-hidden rounded-2xl border border-gray-800 bg-gray-900 shadow-xl animate-fade-up animate-duration-[900ms] animate-delay-200 animate-ease-out">

            @if ($peminjaman->isEmpty())

                {{-- Empty State --}}
                <div class="flex flex-col items-center justify-center px-6 py-20 text-center">
                    <div class="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl border border-gray-800 bg-gray-950/60 text-gray-500 shadow-inner">
                        <svg class="h-7 w-7" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m5.25 11.25h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm-3-6h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm-3-6h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Zm0 3h.008v.008h-.008v-.008Z" />
                        </svg>
                    </div>
            ...
            @else
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left text-sm">
                        <thead class="border-b border-gray-800 bg-gray-950/80 text-xs font-semibold uppercase tracking-wider text-gray-400">
                            <tr>
                            ...
                            @foreach ($peminjaman as $item)
                                <tr class="transition-colors hover:bg-gray-800/50">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center gap-1 font-mono text-xs text-gray-300 rounded-lg border border-gray-800 bg-gray-950/60 px-2.5 py-1">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @if ($st === 'disetujui')
                                            <span class="inline-flex items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-xs font-medium text-emerald-400">
                                                <span class="h-1.5 w-1.5 rounded-full bg-emerald-400"></span>
                                                {{ $item->status->value }}
                                            </span>
                                        @elseif ($st === 'ditolak')
                                            <span class="inline-flex items-center gap-1.5 rounded-full border border-rose-500/30 bg-rose-500/10 px-3 py-1 text-xs font-medium text-rose-400">
                                                <span class="h-1.5 w-1.5 rounded-full bg-rose-400"></span>
                                                {{ $item->status->value }}
                                            </span>
                                        @elseif ($st === 'selesai')
                                            <span class="inline-flex items-center gap-1.5 rounded-full border border-sky-500/30 bg-sky-500/10 px-3 py-1 text-xs font-medium text-sky-400">
                                                <span class="h-1.5 w-1.5 rounded-full bg-sky-400"></span>
                                                {{ $item->status->value }}
                                            </span>
                                        @else
                                            <span class="inline-flex items-center gap-1.5 rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-1 text-xs font-medium text-amber-400">
